...
export function Tutorial({ onStart, onSkip }) {
  ...
  const complete = () => { setPlaced(true); setVerified(false) }
    ...
    <main className="tutorial-screen">
      ...
      <section className="tutorial-card">
        ...
        <div className="demo-area">
          <motion.div className={`demo-grid ${verified ? 'solved' : ''}`} animate={verified ? { scale: [1, 1.03, 1] } : {}}>
          ...
          {!placed && <motion.button draggable onClick={complete} className="number-token" animate={{ y: [0, -7, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}>2</motion.button>}
          {placed && !verified && <button className="primary-button" onClick={() => setVerified(true)}><Check size={18} /> {t('check')}</button>}
          {verified && <motion.div className="success-note" initial={{ scale: 0 }} animate={{ scale: 1 }}><Check size={18} /> {t('tutorialDone')} <Sparkles size={17} /></motion.div>}
        </div>
      </section>
      <button className="primary-button tutorial-start" onClick={onStart} disabled={!verified}>{t('start')} <span>→</span></button>
    </main>
  )
}
